...
func _ready() -> void:
    ...
    add_command("time", func(time): Engine.time_scale = float(time), 1)
    add_command("m", multiple, 2)

func multiple(command, num):
    for i in int(num):
        Console.do_command(command)
...
func do_command(text: String):
    on_text_entered(text, false)

func on_text_entered(text : String, add_history = true) -> void:
    var success = true
    scroll_to_bottom()
    reset_autocomplete()
    line_edit.clear()
    ...
    print_line(text)
    var split_text := parse_command_line(text)
    if (split_text.size() > 0):
        ...
        if (console_commands.has(command_string)):
            ...
            success = false
    if add_history and success:
        save()

func parse_command_line(input: String) -> Array[String]:
    var arguments: Array[String] = []
    var current_argument: String = ""
    var inside_quotes: bool = false

    for word in input.split(" "):
        if inside_quotes:
            # Append to the current argument until we find the closing quote
            current_argument += " " + word
            if word.ends_with("'"):
                inside_quotes = false
                # Remove the surrounding quotes
                current_argument = current_argument.substr(1, current_argument.length() - 2)
            ...
            if word.begins_with("'"):
                ...
                # Regular word (not inside quotes)
